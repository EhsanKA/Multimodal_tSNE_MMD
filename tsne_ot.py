...
class TSNE:
    def __init__(self, X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, initialization=None):
...
import ot
import ot.plot
def tsne(X1, X2, no_dims=30, perplexity=70, max_iter=1000, mmd_coef = 1.0, mmd_radius=1.0, mmd_kernel= t_distribution_kernel_matrix, initialization1=None, initialization2=None):
    """
        Runs t-SNE on the dataset in the NxD array X to reduce its
        dimensionality to no_dims dimensions. The syntaxis of the function is
        `Y = tsne.tsne(X, no_dims, perplexity), where X is an NxD NumPy array.
    """
    ts1 = TSNE(X1, no_dims=no_dims, perplexity=perplexity, initialization=initialization1)
    ts2 = TSNE(X2, no_dims=no_dims, perplexity=perplexity, initialization=initialization2)

    # Run iterations
    for iter in range(max_iter):
        n_source = ts1.Y.shape[0]
        n_target = ts2.Y.shape[0]
        reg = 1
        numItermax = 1000
...
def generate_results(X1, X2, n_repeat=1, no_dims=30, perplexity=70, max_iter=300,
                     ...
                     initialization1=None, initialization2=None):
    ts1_list = []
    ts2_list = []
    time_list = []
    ...
    for i in range(n_repeat):
        ...
        ts1, ts2 = tsne(X1, X2, no_dims=no_dims, perplexity=perplexity, max_iter=max_iter,
                        ...
                        initialization1=initialization1 , initialization2=initialization2)
        end = time.time()
